...
def tree_to_dict(tree, feature_names=None):
    ...
    def recurse(node):
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            return {'type': 'split', 'feature_index': int(tree_.feature[node]), 'threshold': float(threshold), 'left': recurse(tree_.children_left[node]), 'right': recurse(tree_.children_right[node])}
        else:
            value = tree_.value[node][0]
            probs = (value / value.sum()).tolist()
            return {'type': 'leaf', 'value': probs}
    return recurse(0)
...
def export_random_forest(model):
    ...
    return {'type': 'random_forest', 'n_estimators': len(trees), 'trees': trees, 'classes': model.classes_.tolist() if hasattr(model, 'classes_') else [0, 1]}
...
def export_gradient_boosting(model):
    ...
    return {'type': 'gradient_boosting', 'n_estimators': len(trees), 'learning_rate': model.learning_rate, 'init_score': float(model.init_.predict(np.array([[0] * model.n_features_in_]))[0]) if hasattr(model.init_, 'predict') else 0.0, 'trees': trees}

def export_scaler(scaler):
    return {'type': 'standard_scaler', 'mean': scaler.mean_.tolist(), 'scale': scaler.scale_.tolist()}
