    ...
    try:
        with open(file_path, 'r') as f:
            ...
            for row in reader:
                ...
                # Handle rows that might be shorter than headers (unlikely but possible)
                if len(row) <= session_idx: continue
                ...
                # Handle metadata if present (it might be in the last column, beyond headers)
                # The error showed extra columns. Let's assume the last column is metadata if it looks like JSON
                if len(row) > len(headers):
                    # Potential metadata in extra columns
                    last_col = row[-1]
