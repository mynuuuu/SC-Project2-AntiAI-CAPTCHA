...
CSV_HEADERS = ['session_id', 'timestamp', 'time_since_start', 'time_since_last_event', 'event_type', 'client_x', 'client_y', 'relative_x', 'relative_y', 'page_x', 'page_y', 'screen_x', 'screen_y', 'button', 'buttons', 'ctrl_key', 'shift_key', 'alt_key', 'meta_key', 'velocity', 'acceleration', 'direction', 'user_agent', 'screen_width', 'screen_height', 'viewport_width', 'viewport_height', 'user_type', 'challenge_type', 'captcha_id', 'metadata_json']
...
def save_session_summary(session_id, events, metadata):
    ...
    if events:
        ...
        duration = 0
    summary = {'session_id': session_id, 'timestamp': datetime.now().isoformat(), 'total_events': len(events), 'duration_ms': duration, 'event_types': event_types, 'metadata': metadata}
    summaries = []
...
@app.route('/')
def index():
    return jsonify({'status': 'online', 'service': 'Behavior Data Collection Server', 'endpoints': {'/save_events': 'POST - Save captured events', '/stats': 'GET - Get collection statistics', '/sessions': 'GET - List recent sessions'}})
...
def save_events():
    try:
        ...
        return (jsonify({'success': True, 'message': f'Saved {count} events for session {session_id}', 'session_id': session_id, 'events_saved': count, 'summary': summary}), 200)
...
def save_captcha_events():
    try:
        ...
        if not os.path.exists(DATA_DIR):
            os.makedirs(DATA_DIR)
        file_mapping = {'rotation_layer': 'rotation_layer.csv', 'rotation1': 'rotation1.csv', 'layer3_question': 'layer3_question.csv'}
        ...
        elif captcha_id in ['captcha1', 'captcha2', 'captcha3'] and classification_result is not None:
            print(f'    Skipping save for {captcha_id}: classified as BOT (session: {session_id})')
        response = {'success': True, 'message': f'Saved {events_saved} events to {csv_filename}' if events_saved > 0 else 'Events not saved due to bot classification or empty events', 'captcha_id': captcha_id, 'session_id': session_id, 'events_saved': events_saved, 'file_path': csv_path}
...
def get_stats():
    try:
        csv_path = os.path.join(DATA_DIR, CSV_FILENAME)
        if not os.path.exists(csv_path):
            return jsonify({'total_events': 0, 'total_sessions': 0, 'message': 'No data collected yet'})
        ...
        return jsonify({'total_events': total_events, 'total_sessions': total_sessions, 'user_type_distribution': user_types, 'data_file': csv_path})
    except Exception as e:
        return (jsonify({'error': str(e)}), 500)
...
def list_sessions():
    try:
        ...
        return jsonify({'total_sessions': len(summaries), 'recent_sessions': recent_sessions})
    except Exception as e:
        return (jsonify({'error': str(e)}), 500)
...
def export_session(session_id):
    try:
        ...
        csv_content = '\n'.join(output)
        return (csv_content, 200, {'Content-Type': 'text/csv', 'Content-Disposition': f'attachment; filename=session_{session_id}.csv'})
    except Exception as e:
        return (jsonify({'error': str(e)}), 500)
